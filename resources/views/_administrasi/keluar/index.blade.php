@extends('dashboard')
@section('title', 'Administrasi Kas Keluar')
@section('content')  
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2"><i class="fa fa-sign-out" aria-hidden="true"></i> @yield('title')  <a href="{{ route('administrasi.create', 'keluar') }}" class="btn btn-sm btn-link">Form Administrasi</a></h1>
                    ...
                    <table class="table table-sm table-hover" style="width: 100%;">
                        ...
                        <tbody style="display: block;height: 140px;overflow-y: auto;overflow-x: hidden;">
                            ...
                            @foreach($sum_group AS $key => $rows)
                                @php
                                    $sum_keluar += $rows->kas_keluar;
                                @endphp
                                <tr>
                                    <td style="width:80px">{{ $rows->coa }}</td>
                                    <td style="width:290px">{{ $rows->keterangan }}</td>
                                    <td align="right">{{ number_format($rows->kas_keluar) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                        <tfoot style="display: block;">
                            <tr>
                                <td style="width:280px" align="right" colspan="2"><a href="{{ route('administrasi.keluar.print') }}?@php echo @$_SERVER['QUERY_STRING']; @endphp&alokasi=gor"><i class="fas fa-print"></i></a> <b>Total</b></td>
                                <td align="right">{{ number_format($sum_keluar) }} </td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
            ...
            <div class="card-body" style="height:225px;">
                <div class="table-responsive">
                    <table class="table table-sm table-hover" style="width: 100%;">
                        ...
                        <tfoot style="display: block;">
                            <tr>
                                <td style="width:280px" align="right" colspan="2"><a href="{{ route('administrasi.keluar.print') }}?@php echo @$_SERVER['QUERY_STRING']; @endphp&alokasi=butterfly"><i class="fas fa-print"></i></a><b>Total</b></td>
                                <td align="right">{{ number_format($sum_keluar_2) }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
...
<div class="row">
    ...
    <div class="col-md-12">
        ...
        @endif
             <a href="{{ route('administrasi.keluar') }}" class="btn-close float-end" aria-label="Close"></a>
        </p>
                    ...
                    <table class="table table-sm table-hover table-striped text-nowrap">
                        ...
                        @foreach($administrasi AS $rows)
                            <tr>
                                ...
                                <td><a class="btn btn-sm  btn-secondary" href="{{ route('administrasi.keluar.show', $rows->id) }}"><i class="fa-solid fa-pencil "></i></a></td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
