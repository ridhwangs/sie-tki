...
<div class="card">
    <div class="card-body">
        <form action="{{ route('coa.update', $coa->id) }}" id="form" method="POST" autocomplete="off">
        @method('PUT')
        @csrf
            <div class="form-group row mb-2">
                <label for="coa" class="col-sm-2 col-form-label">C.O.A</label>
                <div class="col-sm-10">
                    <input type="text" name="coa" id="coa" class="form-control form-control-sm" value="{{ $coa->coa }}" placeholder="Chart Of Account" autofocus="yes" required>
                </div>
            </div>
            <div class="form-group row mb-2">
                <label for="keterangan" class="col-sm-2 col-form-label">Keterangan</label>
                <div class="col-sm-10">
                    <input type="text" name="keterangan" id="keterangan" class="form-control form-control-sm" value="{{ $coa->keterangan }}" placeholder="Keterangan" required>
                </div>
            </div>
            <div class="form-group row mb-2">
                ...
                <div class="col-sm-10">
                     <select name="alokasi" id="alokasi" class="form-control form-control-sm" required>
                        <option value="" disabled>-- pilih alokasi --</option>
...
<script>
    $(function() {
        $("#transaksi").val("{{ $coa->transaksi }}");
        $("#alokasi").val("{{ $coa->alokasi }}");
    });
</script>
@stop